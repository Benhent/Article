...
export default function ArticleCreate() {
  ...
  const validateForm = () => {
    ...
    if (!formData.isDraft && !articleFile) {
  ...
  const handleSubmit = async (asDraft = true) => {
    // Nếu là bản nháp, chỉ cần tiêu đề
    if (asDraft) {
      if (!formData.title.trim()) {
        setFormErrors((prev) => ({ ...prev, title: "Tiêu đề không được để trống" }))
        return
      ...
      // Validate toàn bộ form nếu không phải bản nháp
    ...
    try {
      ...
      const articleData: any = {
        // Đảm bảo các trường cơ bản luôn được gửi
        title: formData.title,
        titlePrefix: formData.titlePrefix || "", // Đảm bảo titlePrefix luôn có giá trị, dù là chuỗi rỗng
        ...
        // Xử lý từ khóa
        keywords: formData.keywords.length > 0 ? formData.keywords : !asDraft ? ["Chưa phân loại"] : [],
      ...
      if (articleFile) {
        try {
          ...
          const { fileUrl, fileName } = await uploadArticleFileToCloudinary(articleFile)
          articleData.articleFile = fileUrl
          articleData.articleFileName = fileName
      ...
      // Gửi request với dữ liệu JSON
      await createArticle(articleData)
      setUploadProgress(100)

      showSuccessToast(asDraft ? "Đã lưu bản nháp thành công" : "Đã nộp bài báo thành công")
    ...
    <div className="container mx-auto py-8">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          ...
          {/* Dialog xác nhận nộp bài - tách riêng để tránh lỗi accessibility */}
          <Dialog open={showConfirmation} onOpenChange={setShowConfirmation}>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Xác nhận nộp bài báo</DialogTitle>
                <DialogDescription>
                  Bạn có chắc chắn muốn nộp bài báo này? Sau khi nộp, bài báo sẽ được chuyển đến ban biên tập để xem
                  xét.
                </DialogDescription>
              </DialogHeader>
              <DialogFooter>
                ...
                <Button onClick={() => handleSubmit(false)}>Nộp bài báo</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
        </div>
                            ...
                            <dd className="mt-1">
                              {articleFile ? (
                                ...
                                <span className="text-yellow-600">
                                  ...
                                  {!formData.isDraft && <span className="font-medium">(bắt buộc khi nộp bài)</span>}
                                </span>
                              )}
                            </dd>
                          </div>
                    ...
                    <div className="bg-blue-50 p-6 rounded-md">
                      ...
                      <div className="flex items-center space-x-2 mb-4">
                        ...
                        <Label htmlFor="isDraft" className="font-medium">
                          Tôi muốn nộp bài báo này (không phải lưu bản nháp)
                        </Label>
                      </div>
                      ...
                      <div className="flex flex-col sm:flex-row gap-3 mt-6">
                        ...
                        <Button
                          ...
                          disabled={isSubmitting || formData.isDraft}
                          className="flex-1"
                        >
                          {isSubmitting ? (
                            <Loader2 className="mr-2 h-4 w-4 animate-spin" />
